<div class="space-y-8">
    ...
    <div class="bg-white/40 backdrop-blur-md border border-white/50 rounded-3xl p-6 shadow-sm transition-all duration-300 hover:bg-white/60 hover:shadow-md flex flex-col">
        ...
        <div class="space-y-3 max-h-[500px] overflow-y-auto pr-2" id="videoListContainer">
            @forelse($videos as $v)
                @php
                    $rawStatus = strtoupper($v->status_kondisi);
                    
                    // SINKRONISASI COATING VISUAL BADGE & TEXT STATUS BARU
                    if ($rawStatus === 'NORMAL' || $rawStatus === 'AMAN') {
                        $displayStatus = 'SIAGA';
                        $badgeColor = 'bg-emerald-100 text-emerald-700 border border-emerald-200';
                    } elseif ($rawStatus === 'SIAGA' || $rawStatus === 'WASPADA') {
                        $displayStatus = 'WASPADA';
                        $badgeColor = 'bg-amber-100 text-amber-700 border border-amber-200';
                    } else {
                        $displayStatus = 'AWAS';
                        $badgeColor = 'bg-rose-100 text-rose-700 border border-rose-200 animate-pulse';
                    }
                @endphp
